...
| mouse click-to-cursor | pi-tui `Editor.handleMouse` | pi-tui's editor already implements click-to-cursor; fullscreen mode dispatches mouse events with component-local coordinates. BoxEditor overrides `handleMouse` only to translate clicks from its extended layout (style rows above the text, prompt inset) into the base editor's coordinate space. No mouse reporting is enabled by pi-ui itself. |
