/**
 * Welcome screen header for @mightguy/pi-ui.
 *
 * Ported from pi-kaush/extensions/pi-welcome-screen.
 * Shows a custom ASCII banner + pi resource sections (Context, Skills,
 ...
 *   Pi builds its own loaded-resource panel just after session_start. We
 *   temporarily remove that native panel from the TUI tree (takeResourcePanel),
 *   parse its text content to extract context / skills / prompts / extensions,
 *   then render our own grid layout. If parsing fails or an unknown section is
 *   found, we put the native panel back (restoreResourcePanel).
 */
...
const RESOURCE_BRIDGE_KEY = "__mightguyWelcomeScreenResourceBridge";
...
function isResourcePanel(c: Component | undefined): c is ResourcePanel {
  if (!c || typeof c !== "object") return false;
  return Array.isArray((c as Partial<Container>).children);
}
...
function centerBlockLine(line: string, blockWidth: number, width: number): string {
  const clipped = truncateToWidth(line, width, "");
  return " ".repeat(Math.max(0, Math.floor((width - blockWidth) / 2))) + clipped;
}
...
function wrapPrefixed(prefix: string, text: string, width: number): string[] {
  ...
  return wrapped.map((line, i) => `${i === 0 ? prefix : continuation}${line}`);
}
...
function isWelcomeScreenExtension(name: string): boolean {
  ...
  return (
    ...
    name === "@mightguy/pi-ui" ||
    name === "src" ||
    /\/(?:pi-)?welcome-screen(?:\/|$)/.test(n)
  );
}

export function sortExtensionNames(names: string[]): string[] {
  return [...names].sort((a, b) => {
    const order = Number(a.startsWith("@")) - Number(b.startsWith("@"));
...
function splitGitRevision(s: string): { path: string; revision: string | undefined } {
  const sep = s.indexOf("@");
  if (sep === -1) return { path: s, revision: undefined };
  return { path: s.slice(0, sep), revision: s.slice(sep + 1) };
}

function parseGitPackageSource(source: string): { label: string; revision: string | undefined } {
  const value = source.replace(/^git:/, "");
  const scpMatch = value.match(/^git@([^:]+):(.+)$/);
  if (scpMatch) {
    const { path, revision } = splitGitRevision(scpMatch[2] ?? "");
    return { label: `${scpMatch[1] ?? ""}/${path}`.replace(/\.git$/, ""), revision };
  }
  if (value.includes("://")) {
    try {
      const url = new URL(value);
      const { path, revision } = splitGitRevision(url.pathname.replace(/^\/+/, ""));
      return { label: `${url.hostname}/${path}`.replace(/\.git$/, ""), revision };
    } catch { return { label: value, revision: undefined }; }
  }
  const slash = value.indexOf("/");
  if (slash === -1) return { label: value, revision: undefined };
  const { path, revision } = splitGitRevision(value.slice(slash + 1));
  return { label: `${value.slice(0, slash)}/${path}`.replace(/\.git$/, ""), revision };
}

function formatPackageExtensionLabel(source: string, extensionPaths: string[]): string {
  const extensionNames = unique(
    extensionPaths
      .map((p) => p.replace(/\\/g, "/").split("/").pop() ?? "")
      .filter((n) => /\.[cm]?[jt]s$/.test(n)),
  );
  if (!source.startsWith("git:")) {
    return [normalizePackageSource(source), ...extensionNames].join(" ");
  }
  const { label, revision } = parseGitPackageSource(source);
  return [label, ...extensionNames, ...(revision ? [`@${revision.slice(0, 6)}`] : [])].join(" ");
}

function isExplicitSourcePath(label: string): boolean {
  const n = label.replace(/\\/g, "/");
  return (
    n.startsWith("/") || n.startsWith("~/") || n.startsWith("./") ||
    n.startsWith("../") || /^[A-Za-z]:\//.test(n) ||
    /\/(?:index)\.(?:[cm]?[jt]s)$/.test(n)
  );
}

// ─── Extension group parsing ──────────────────────────────────────────────────
...
function parseExpandedExtensionGroups(
  ...
  const flushPackage = () => {
    ...
    packageExtensions.push(formatPackageExtensionLabel(currentPackageSource, currentPackagePaths));
  ...
  for (const rawLine of text.split("\n").slice(1)) {
    ...
    if (packageSource) { flushPackage(); currentPackageSource = packageSource; foundItem = true; continue; }
    const packagePath = line.match(/^ {6}(\S.*)$/)?.[1];
    if (packagePath && currentPackageSource) { currentPackagePaths.push(packagePath); continue; }
    flushPackage();
    ...
    if (
      ...
      /(?:^|\/)\.pi\/(?:agent\/)?extensions(?:\/|$)/.test(path.replace(/\\/g, "/"))
    ) {
...
function classifyCompactExtensionLabels(
  ...
  for (const label of labels) {
    ...
    const indexParent = label.replace(/\\/g, "/").match(/(?:^|\/)([^/]+)\/index\.(?:[cm]?[jt]s)$/)?.[1];
    if (localExtensionNames.has(name) || localExtensionNames.has(indexParent ?? ""))
      localExtensions.push(name);
    else if (isPackageSource(label) || name.startsWith("@"))
      packageExtensions.push(isPackageSource(label) ? normalizePackageSource(label) : name);
  ...
  try {
    cachedLocalExtensionNames = new Set(
      readdirSync(extensionsDir, { withFileTypes: true }).flatMap((entry) => {
        if (/\.[cm]?[jt]s$/.test(entry.name)) return normalizeExtensionName(entry.name);
...
function inspectResourcePanel(panel: ResourcePanel): ResourcePanelSnapshot {
  ...
  for (const child of panel.children) {
    ...
    if (typeof collapsible.getCollapsedText === "function") {
      ...
      if (WELCOME_SECTIONS.some((s) => s === heading)) {
        sections.push(text);
        if (
          (heading === "Extensions" || /(?:^|\n)\s*\[Extensions\]\s*(?:\n|$)/.test(stripAnsi(text))) &&
    ...
    const hasVisibleContent = child.render(1_000).some((line) => stripAnsi(line).trim().length > 0);
    if (hasVisibleContent) return { resourceText: "", requiresNativePanel: true };
  }
...
export function parseWelcomeResources(
  ...
  for (const rawLine of text.split("\n")) {
    ...
    if (header) {
      currentSection = WELCOME_SECTIONS.find((s) => s === header);
      if (currentSection && !bodies.has(currentSection)) bodies.set(currentSection, []);
      continue;
    }
  ...
  return {
    context: unique(splitList(bodies.get("Context") ?? [])),
    skills: unique(splitList(bodies.get("Skills") ?? [])),
    prompts: unique(splitList(bodies.get("Prompts") ?? [])),
    extensions: [...groups.localExtensions, ...groups.packageExtensions, ...groups.sourceExtensions],
    packageExtensions: groups.packageExtensions,
    sourceExtensions: groups.sourceExtensions,
  };
}

// ─── Resource panel bridge ────────────────────────────────────────────────────
...
function takeResourcePanel(tui: TUI): ResourceBridge | undefined {
  ...
  if (tui.children.length < 8 || !isResourcePanel(tui.children[0])) return undefined;
...
function restoreResourcePanel(tui: TUI, bridge: ResourceBridge | undefined): void {
  if (!bridge) return;
...
function appendSingleColumnRows(lines: string[], items: string[], theme: Theme, columnWidth: number): void {
  for (const item of items)
    lines.push(...wrapPrefixed(theme.fg("dim", "  • "), theme.fg("dim", item), columnWidth));
}
...
function getColumnWidths(listWidth: number, columnCount: number): number[] {
  ...
  return Array.from({ length: columnCount }, (_, i) => baseCellWidth + (i < widerCellCount ? 1 : 0));
}

function canUseThreeColumns(items: string[], columnWidth: number): boolean {
  const listWidth = Math.max(1, columnWidth - 2);
  const fittingColumns = Math.floor((listWidth + LIST_COLUMN_GAP) / (MIN_LIST_COLUMN_WIDTH + LIST_COLUMN_GAP));
  ...
  return items.every((item, i) => {
    ...
    return visibleWidth(`• ${item}`) <= (cellWidths[col] ?? 0);
  });
}

function getSharedMultiColumnCount(resources: WelcomeResources, columnWidth: number): 2 | 3 {
  const packageExtensions = new Set(
    resources.packageExtensions ?? resources.vendoredExtensions ?? resources.extensions.filter((n) => n.startsWith("@")),
  );
  const sourceExtensions = new Set(resources.sourceExtensions ?? []);
  const localExtensions = resources.extensions.filter((n) => !packageExtensions.has(n) && !sourceExtensions.has(n));
  const multiColumnLists = [resources.skills, localExtensions].filter((items) => items.length > MAX_LIST_ROWS_PER_COLUMN);
  return multiColumnLists.every((items) => canUseThreeColumns(items, columnWidth)) ? 3 : 2;
}
...
function appendColumnRows(
  ...
  const fittingColumns = Math.max(
    1,
    Math.floor((listWidth + LIST_COLUMN_GAP) / (MIN_LIST_COLUMN_WIDTH + LIST_COLUMN_GAP)),
  );
  const requestedColumns = sharedColumnCount && items.length > MAX_LIST_ROWS_PER_COLUMN ? sharedColumnCount : desiredColumns;
  const columnCount = Math.min(requestedColumns, fittingColumns);

  if (columnCount === 1) { appendSingleColumnRows(lines, items, theme, columnWidth); return; }

  const rowsPerColumn = Math.ceil(items.length / columnCount);
  const cellWidths = getColumnWidths(listWidth, columnCount);

  for (let row = 0; row < rowsPerColumn; row++) {
    const cells = cellWidths.map((cellWidth, col) => {
      const item = items[col * rowsPerColumn + row];
    ...
    lines.push(theme.fg("dim", `  ${cells.join(" ".repeat(LIST_COLUMN_GAP))}`.trimEnd()));
  }
}
...
function appendSection(
  ...
  if (body.length === 0) { lines.push(theme.fg("dim", "  (none)")); return; }
...
function appendExtensionsSection(
  ...
  if (extensions.length === 0) { lines.push(theme.fg("dim", "  (none)")); return; }

  const packageExtensions = new Set(packageExtensionNames ?? extensions.filter((n) => n.startsWith("@")));
  const sourceExtensions = new Set(sourceExtensionNames ?? []);
  const localExtensions = extensions.filter((n) => !packageExtensions.has(n) && !sourceExtensions.has(n));
  const installedPackageExtensions = extensions.filter((n) => packageExtensions.has(n));
  const linkedSourceExtensions = extensions.filter((n) => sourceExtensions.has(n));

  const groups = [
    { title: "Local", items: localExtensions, multiColumn: true },
    { title: "Packages", items: installedPackageExtensions, multiColumn: false },
    { title: "Source paths", items: linkedSourceExtensions, multiColumn: false },
  ].filter(({ items }) => items.length > 0);

  for (const [i, group] of groups.entries()) {
    if (i > 0) lines.push("");
    lines.push(theme.fg("muted", `  ${group.title}`));
    if (group.multiColumn) appendColumnRows(lines, group.items, theme, columnWidth, sharedColumnCount);
    else appendSingleColumnRows(lines, group.items, theme, columnWidth);
  }
}
...
function renderBrandColumn(theme: Theme, columnWidth: number): string[] {
  ...
  lines.push(centerBlockLine(versionSummary, visibleWidth(versionSummary), columnWidth));
  return lines;
}
...
function appendResourceSection(
  ...
  const body = title === "Context" ? resources.context : title === "Skills" ? resources.skills : resources.prompts;
  appendSection(lines, title, body, theme, columnWidth, title === "Context", title === "Skills" ? sharedColumnCount : undefined);
}

function renderResourceColumn(resources: WelcomeResources, theme: Theme, columnWidth: number): string[] {
  const lines: string[] = [];
  const sharedColumnCount = getSharedMultiColumnCount(resources, columnWidth);
  for (const title of WELCOME_SECTIONS)
    appendResourceSection(lines, title, resources, theme, columnWidth, sharedColumnCount);
  return lines;
}
...
function renderGridItem(
  ...
  const lines: string[] = [];
  appendResourceSection(lines, item, resources, theme, columnWidth, sharedColumnCount);
  return lines;
}

function renderGridWelcome(resources: WelcomeResources, theme: Theme, columnWidth: number, columnCount: 2 | 3): string[] {
  const sharedColumnCount = getSharedMultiColumnCount(resources, columnWidth);
  ...
  const rowCount = Math.max(...topAlignedColumns.map((col) => col.length));

  if (columnCount === 2) {
    const layoutWidth = columnWidth * 2 + GRID_COLUMN_GAP;
    const resourceRows = Array.from({ length: rowCount }, (_, row) =>
      topAlignedColumns
        .map((col) => padToWidth(col[row] ?? "", columnWidth))
  ...
  const columns = topAlignedColumns.map((col, i) =>
    i === 0
      ? [...Array.from({ length: Math.floor((rowCount - col.length) / 2) }, () => ""), ...col]
      : col,
  );
  return Array.from({ length: rowCount }, (_, row) =>
    columns.map((col) => padToWidth(col[row] ?? "", columnWidth)).join(" ".repeat(GRID_COLUMN_GAP)).trimEnd(),
  );
}

function renderStackedWelcome(resources: WelcomeResources | undefined, theme: Theme, columnWidth: number): string[] {
  const lines = ["", ...renderBrandColumn(theme, columnWidth)];
  if (resources) lines.push("", ...renderResourceColumn(resources, theme, columnWidth));
  lines.push("");
  return lines;
}
...
export function renderCenteredWelcome(resources: WelcomeResources | undefined, theme: Theme, width: number): string[] {
  if (width <= 0) return [];
  const columnCount = resources ? getGridColumnCount(width) : 1;
  const columnWidth =
    columnCount === 1
      ? Math.min(MAX_STACKED_COLUMN_WIDTH, width)
      : Math.min(MAX_GRID_COLUMN_WIDTH, Math.floor((width - GRID_COLUMN_GAP * (columnCount - 1)) / columnCount));
  const layoutWidth = columnWidth * columnCount + GRID_COLUMN_GAP * (columnCount - 1);
  ...
  return lines.map((line) => (line ? leftPadding + truncateToWidth(line, layoutWidth, "") : ""));
}
...
class WelcomeHeader implements Component {
  ...
  private captureResourcesWhenReady(forceInitialRender: boolean, attempt: number): void {
    ...
    if (snapshot.requiresNativePanel) { this.showNativePanel(forceInitialRender); return; }

    const { resourceText, expandedExtensionsText } = snapshot;
    const candidateResources = resourceText
      ? parseWelcomeResources(resourceText, getLocalExtensionNames(), expandedExtensionsText)
      : undefined;
    const resourcePanelIsComplete = Boolean(candidateResources?.extensions.some(isWelcomeScreenExtension));
  ...
  render(width: number): string[] {
    ...
    if (resources) { this.cachedWidth = width; this.cachedLines = lines; }
    return lines;
  }
...
/**
 * Factory for use with ctx.ui.setHeader().
 * Pass `isStartup` as true only on the initial "startup" session_start so the
 * header forces a full repaint on first load.
 */
export function makeWelcomeHeader(
  tui: TUI,
  theme: Theme,
  isStartup: boolean,
): WelcomeHeader {
  return new WelcomeHeader(tui, theme, takeResourcePanel(tui), isStartup);
}
