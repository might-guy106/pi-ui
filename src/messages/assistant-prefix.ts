...
import { dropLeadingColumns, fgHex, startsWithVisibleSpace, stripAnsi } from "../theme/ansi.ts";
import { getThemeExtra } from "../theme/theme-extras.ts";
import { safeTakeTailToWidth, safeTruncateToWidth, safeVisibleWidth } from "../render-budget.ts";
import { attachComponentToStream } from "./assistant-streaming-state.ts";
import {
	type AssistantContentRun,
	getAssistantContentRuns,
} from "./assistant-content-runs.ts";

let activeTheme: any = null;
const PATCHED = Symbol.for("pi-ui.assistant-prefix.patched");
...
export function installAssistantMessagePrefix(theme: any, componentClass: any = AssistantMessageComponent): void {
	...
	proto.render = function patchedAssistantMessageRender(width: number): string[] {
		...
		if (this.__assistantResponsePrefixChildMode) {
			const result = lines.map((renderedLine: string) =>
				safeVisibleWidth(renderedLine) > width ? safeTruncateToWidth(renderedLine, width, "") : renderedLine,
			);
