/**
 * Startup UI, ported from pi-droid-styling's startup-ui.ts:
 * - gradient "pi" logo header (via ctx.ui.setHeader) with key hints + ready
 * - compact "◆ Resources …" summary in the chat that replaces pi's native
 *   resource listing (expandable with ctrl+o into two tables)
 * - console.log filter for pi's "Model scope" startup line
 *
...
import { getAgentDir, keyHint, rawKeyHint, VERSION, type ExtensionUIContext } from "@earendil-works/pi-coding-agent";
...
type ThemeLike = StartupThemeLike;
...
// Resource-summary constants (from upstream startup-ui.ts).
const PATCHED = Symbol.for("pi-ui.startup-ui.patched");
const ORIGINAL_SHOW_LOADED_RESOURCES = Symbol.for("pi-ui.startup-ui.original-show-loaded-resources");
...
// ---------------------------------------------------------------------------
// Compact resource summary (ported from pi-droid-styling's startup-ui.ts):
// replaces pi's native resource listing with a one-line "◆ Resources …"
// summary in the chat, expandable (ctrl+o) into System & Context and
